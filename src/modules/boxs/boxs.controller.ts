...
export class BoxsController {
    ...
    public async getTableTest( @Res() res: Response, @Param('id_public') id) {
        ...
        if (j == -1) {
            ...
            if (state) {
                ...
                return res.json({ turno: -1, description: "El usuario no existe" });
            }
        } else {
    ...
    public async getScore( @Res() res: Response) {
        let table = await this.scoreService.getAllScore();
        if (this.turno >= 0) {
            let interva2 = setInterval((function(self) {         
                return async function() { 
                    if(self.turno > self.copy_turno2 || self.turno == 0){
                        self.copy_turno2 = self.turno;
                        clearInterval(interva2);
                        return res.status(HttpStatus.OK).json({ turno: self.turno, tablero: table });

                    } else if(self.turno == -1){
                        clearInterval(interva2);
                        return res.json({ turno: self.turno, tablero: table });
                    }
                }
             })(this), 1000);
        } else {
            return res.status(HttpStatus.OK).json({ turno: this.turno, tablero: table });
        }
    }
